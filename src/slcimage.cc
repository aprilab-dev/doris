...
//TODO UPDATE and ADD the FM rate
    ...
    // ___ if you want to force behavior, change this in master.res ___
      ...
      else if ( strstr(word,"PAZ") != NULL ) // PAZ [FvL]
        {
          sar_processor = SARPR_TSX;
          DEBUG.print("SAR_PROCESSOR: TSX identified for PAZ.");
        }
...
//        }
      ...
      pch = strstr(word,"ALOS2");
      if (pch != NULL) //ALOS2 bij YQ
        {
      ...
      if (pch != NULL)  // FC1
        {
          DEBUG.print("Substring \"FC1\" found in Product type specifier.");
          sensor = SLC_FC1;
        }
      pch = strstr(word,"BC3");
      if (pch != NULL)  // BC3
        {
          DEBUG.print("Substring \"BC3\" found in Product type specifier.");
          sensor = SLC_BC3;
        }
      pch = strstr (word,"TSX");
      if (pch != NULL)
      ...
      pch = strstr (word,"PAZ");  // PAZ [FvL]
      if (pch != NULL)
        {
          DEBUG.print("Substring \"PAZ\" found in Product type specifier.");
          sensor = SLC_TSX;
        }
  ...
  if (sensor==SLC_TSX)
    {
      INFO.print("Yeah, TerraSAR-X, TanDEM-X or PAZ!");
      if (abs(wavelength-0.031) > 0.01)
        WARNING.print("wavelength seems to deviates more than 1 cm from TSX/TDX/PAZ nominal.");
    }
  if (sensor==SLC_RS2)
    {
