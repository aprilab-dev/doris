 ...
 *    alos2_dump_data()                                         *
 *                                                              *
 * Via a system call to the python alos2_dump_data()            *
 * alos2_dump_data() writes SLC data out in host order.         *
 * it is important that crop_arg.dbow is correctly filled.      *
 *                                                              *
...
{
  // ______ Write some info ______
  TRACE_FUNCTION("alos2_dump_data (07-Feb-2019)")
    // ______ Build command ______
    // ______ make sure l0 etc. are correctly defined ______
    // ____ assume these are filled correctly ___
    int16 status = 0;    // [MA] check exit status of system calls for proper error handling
  ...
  if (crop_arg.dbow.linehi!=0 && crop_arg.dbow.linelo!=0 &&
      crop_arg.dbow.pixhi!=0 && crop_arg.dbow.pixlo!=0)
    INFO << "alos2_dump_data.py " << crop_arg.filein1
         << " " << crop_arg.fileout1
         //<< " " << crop_arg.dbow.linelo - 1
         << " " << crop_arg.dbow.linelo
         << " " << crop_arg.dbow.linehi
  ...
  char cmd[512];// command string
  strcpy(cmd, INFO.get_str());
  INFO.print("With following command ALOS2 CEOS data was cropped.");
  INFO.print(cmd);
  PROGRESS.print("system call may take some time...");
  status=system(cmd);// this does the work
  if (status != 0)                                                          // [MA] TODO make it a function
    {
    ERROR << "alos2_dump_data.py: failed with exit code: " << status;
    PRINT_ERROR(ERROR.get_str())
    throw(some_error);
    }
  INFO.reset();
  ...
  scratchresfile
    ...
    <<  crop_arg.fileout1
    << "\nData_output_format: \t\t\t\t"
    << "complex_real4" //FvL, changed from complex_short, complex_real4 is original Alos2 format
    // ______ updateslcimage greps these ______
    << "\nFirst_line (w.r.t. original_image): \t\t"
    <<  crop_arg.dbow.linelo
    << "\nLast_line (w.r.t. original_image): \t\t"
 ...
 *    lt1_dump_data()                                           *
 *                                                              *
 * Via a system call to the python lt1_dump_data()              *
 * lt1_dump_data() writes SLC data out in host order.           *
 * it is important that crop_arg.dbow is correctly filled.      *
 *                                                              *
 * Dependencies: TBA                                *
 * yuxiao                                                       *
...
{
  // ______ Write some info ______
  TRACE_FUNCTION("lt1_dump_data (27-Jan-2025)")
    // ______ Build command ______
    // ______ make sure l0 etc. are correctly defined ______
    // ____ assume these are filled correctly ___
    int16 status = 0;    // [MA] check exit status of system calls for proper error handling
  ...
  if (crop_arg.dbow.linehi!=0 && crop_arg.dbow.linelo!=0 &&
      crop_arg.dbow.pixhi!=0 && crop_arg.dbow.pixlo!=0)
    INFO << "lt1_dump_data.py " << crop_arg.filein1
         << " " << crop_arg.fileout1
         //<< " " << crop_arg.dbow.linelo - 1
         << " " << crop_arg.dbow.linelo
         << " " << crop_arg.dbow.linehi
  ...
  char cmd[512];// command string
  strcpy(cmd, INFO.get_str());
  INFO.print("With following command LT1 data was cropped.");
  INFO.print(cmd);
  PROGRESS.print("system call may take some time...");
  status=system(cmd);// this does the work
  if (status != 0)                                                          // [MA] TODO make it a function
    {
    ERROR << "lt1_dump_data.py: failed with exit code: " << status;
    PRINT_ERROR(ERROR.get_str())
    throw(some_error);
    }
  INFO.reset();
 ...
 *    fc1_dump_data()                                           *
 *                                                              *
 * Via a system call to the python fc1_dump_data()              *
 * fc1_dump_data() writes SLC data out in host order.           *
 * it is important that crop_arg.dbow is correctly filled.      *
 *                                                              *
 * yuxiao                                                       *
 ****************************************************************/
...
{
  // ______ Write some info ______
  TRACE_FUNCTION("fc1_dump_data (02-Jan-2024)")
    // ______ Build command ______
    // ______ make sure l0 etc. are correctly defined ______
    // ____ assume these are filled correctly ___
    int16 status = 0;    // [MA] check exit status of system calls for proper error handling
  ...
  if (crop_arg.dbow.linehi!=0 && crop_arg.dbow.linelo!=0 &&
      crop_arg.dbow.pixhi!=0 && crop_arg.dbow.pixlo!=0)
    INFO << "fc1_dump_data.py " << crop_arg.filein1
         << " " << crop_arg.fileout1
         //<< " " << crop_arg.dbow.linelo - 1
         << " " << crop_arg.dbow.linelo
         << " " << crop_arg.dbow.linehi
  ...
  char cmd[512];// command string
  strcpy(cmd, INFO.get_str());
  INFO.print("With following command FC1 data was cropped.");
  INFO.print(cmd);
  PROGRESS.print("system call may take some time...");
  status=system(cmd);// this does the work
  if (status != 0)                                                          // [MA] TODO make it a function
    {
    ERROR << "fc1_dump_data.py: failed with exit code: " << status;
    PRINT_ERROR(ERROR.get_str())
    throw(some_error);
    }
  INFO.reset();
 ...
 *    bc3_dump_data()                                           *
 *                                                              *
 * Via a system call to the python bc3_dump_data()              *
 * bc3_dump_data() writes SLC data out in host order.           *
 *                                                              *
 * yuxiao                                                       *
 ****************************************************************/
void bc3_dump_data(
...
{
  // ______ Write some info ______
  TRACE_FUNCTION("bc3_dump_data (22-Feb-2024)")
    // ______ Build command ______
    // ______ make sure l0 etc. are correctly defined ______
    // ____ assume these are filled correctly ___
    int16 status = 0;    // [MA] check exit status of system calls for proper error handling
  ...
  if (crop_arg.dbow.linehi!=0 && crop_arg.dbow.linelo!=0 &&
      crop_arg.dbow.pixhi!=0 && crop_arg.dbow.pixlo!=0)
    INFO << "bc3_dump_data.py " << crop_arg.filein1
         << " " << crop_arg.fileout1
         //<< " " << crop_arg.dbow.linelo - 1
         << " " << crop_arg.dbow.linelo
         << " " << crop_arg.dbow.linehi
  ...
  char cmd[512];// command string
  strcpy(cmd, INFO.get_str());
  INFO.print("With following command BC3 data was cropped.");
  INFO.print(cmd);
  PROGRESS.print("system call may take some time...");
  status=system(cmd);// this does the work
  if (status != 0)                                                          // [MA] TODO make it a function
    {
    ERROR << "bc3_dump_data.py: failed with exit code: " << status;
    PRINT_ERROR(ERROR.get_str())
    throw(some_error);
    }
  INFO.reset();
